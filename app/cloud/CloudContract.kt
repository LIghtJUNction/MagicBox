...
internal fun requireCloud(value: Boolean, message: String) {
    if (!value) throw CloudFailure(message)
}
...
internal fun machineData(text: String, command: String): JSONObject {
    val envelope = runCatching { JSONObject(text.trim()) }.getOrNull()
    requireCloud(envelope != null && envelope.optInt("schema", -1) == 1 &&
        envelope.optBoolean("ok", false) && envelope.optString("command") == command,
...
internal fun safeNodeDocument(document: JSONObject): JSONObject {
    ...
    val all = (copy.optJSONArray("outbounds") ?: JSONArray()).objects() +
        (copy.optJSONArray("endpoints") ?: JSONArray()).objects()
    requireCloud(all.isNotEmpty() && all.size <= 5000, "节点数量无效，最多支持 5000 个节点。")
    val tags = mutableSetOf<String>()
    all.forEachIndexed { index, node ->
        val type = node.optString("type")
        requireCloud(type.isNotBlank(), "节点缺少协议类型。")
    ...
    fun validate(value: Any?, depth: Int) {
        ...
        when (value) {
            is JSONObject -> value.keys().forEach { key ->
                requireCloud(!key.endsWith("_path") && key !in setOf("providers", "script", "command"),
                    "订阅包含本地文件或可执行引用，已拒绝导入。")
...
internal fun nodeChoices(document: JSONObject): List<JSONObject> =
    ((document.optJSONArray("outbounds") ?: JSONArray()).objects() +
        (document.optJSONArray("endpoints") ?: JSONArray()).objects())
...
internal fun makeCoreConfig(document: JSONObject, mode: String, selected: String, secret: String, cgroup: String): JSONObject {
    ...
    outbounds.put(JSONObject().put("type", "selector").put("tag", CLOUD_SELECTOR)
        .put("outbounds", JSONArray(nodes.map { it.optString("tag") })).put("default", selected))
    ...
    val inbounds = JSONArray().put(JSONObject().put("type", "mixed").put("tag", "mixed-in")
        .put("listen", "127.0.0.1").put("listen_port", CLOUD_PROXY_PORT))
    if (mode == "tun") {
    ...
    if (mode == "ebpf") {
        inbounds.put(JSONObject().put("type", "ebpf").put("tag", "tun-in").put("mode", "local")
            .put("network", JSONArray(listOf("tcp", "udp")))
            .put("local", JSONObject().put("dns_mode", "hijack").put("cgroup_path", cgroup)
    ...
    return JSONObject().put("log", JSONObject().put("level", "warn").put("timestamp", false))
        ...
        .put("endpoints", document.optJSONArray("endpoints") ?: JSONArray())
        ...
        .put("route", JSONObject().put("auto_detect_interface", true).put("default_domain_resolver", CLOUD_DNS)
            ...
            .put("final", CLOUD_SELECTOR))
        .put("experimental", JSONObject().put("clash_api", JSONObject()
            .put("external_controller", "127.0.0.1:$CLOUD_API_PORT").put("secret", secret)))
}
