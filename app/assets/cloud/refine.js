...
(() => {
 ...
 function motionState(){let manual=false;try{manual=localStorage.getItem('cloud.reduce')==='true';}catch(_){}return{manual,media:reduce.matches,hidden:document.hidden};}
 function quiet(){const value=motionState();return value.hidden||value.media||value.manual;}
 ...
 if(rootStatus){const normalize=()=>{const granted=rootStatus.textContent.includes('已授权'),next=granted?'已授权':'未授权';if(rootStatus.textContent!==next)rootStatus.textContent=next;};new MutationObserver(normalize).observe(rootStatus,{childList:true,subtree:true,characterData:true});normalize();}
 const status=document.getElementById('status-detail');if(status?.textContent==='添加订阅，选择自己的连接方式。')status.textContent='';
 document.addEventListener('visibilitychange',()=>{if(document.hidden)stop();});
 addEventListener('resize',()=>{fit();if(active&&!frame)frame=requestAnimationFrame(paint);});
 window.MagicClick={metrics(){return{active,frames,...motionState()};}};
})();
